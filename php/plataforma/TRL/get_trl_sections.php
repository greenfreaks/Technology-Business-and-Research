...
//query code
    ...
    if($exec_query_sections->num_rows>0){
    	...
    	while ($row = $exec_query_sections->fetch_assoc()) {
            ...
            $query_glosary = "SELECT `idtrl_glosario`, `concepto`, `definicion` FROM `trl_glosario` WHERE `trl_categoria_idtrl_categoria`='{$row['idtrl_categoria']}';";
            //echo $query_glosary;
            $exec_query_glosary = $conex->query($query_glosary );
            
            if($exec_query_glosary->num_rows>0){
            	$sections['glosary']=array();
            	while ($row_glosary = $exec_query_glosary->fetch_assoc()) {
            		$glosary=array();
            		$glosary['concepto'] = $row_glosary['concepto'];
            		$glosary['definicion'] = $row_glosary['definicion'];
            		array_push( $sections['glosary'] , $glosary);	
            	}
            	$result['msg'] .= "->data glosary selected ";
            }else{
            	
            	$result['error'] = true;
                $result['msg'] .= "->ERROR selecting glosary";
            }
