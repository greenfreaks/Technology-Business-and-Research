...
class LinearTicks extends Ticks {
    ...
    function _doManualTickPos($aScale) {
        ...
        $m=strlen($this->iManualMinTickPos);
...
class ColorFactory {
	...
	function MultiCellBltArray($w, $h, $blt_array, $border, $fill, $align='J' )
    {
        ...
        for ($i=0; $i<sizeof($blt_array['text']); $i++)
        {
            ...
            $blt_width = $this->GetStringWidth($blt_array['bullet'] . $blt_array['margin'])+$this->cMargin*2+$this->GetStringWidth($blt_array['indent']);
			...
			if($border==1){
				if($i==0){
					...
					$this->Cell($blt_width,$h,$blt_array['indent'].$blt_array['bullet'] . $blt_array['margin'],'LT','','',$fill);
					$this->MultiCell($w+0.8,$h,$blt_array['text'][$i],'RT',$align,$fill);
					
				}elseif($blt_array['text'][$i]==end($blt_array['text'])){
		
					$this->Cell($blt_width,$h,$blt_array['indent'].$blt_array['bullet'] . $blt_array['margin'],'LB','','',$fill);
					$this->MultiCell($w+0.8,$h,$blt_array['text'][$i],'RB',$align,$fill);
					
				}else{
					
					$this->Cell($blt_width,$h,$blt_array['indent'].$blt_array['bullet'] . $blt_array['margin'],'L','','',$fill);
					$this->MultiCell($w+0.8,$h,$blt_array['text'][$i],'R',$align,$fill);
				}
			}else{
				
				$this->Cell($blt_width,$h,$blt_array['indent'].$blt_array['bullet'] . $blt_array['margin'],$border,'','',$fill);
				$this->MultiCell($w+0.8,$h,$blt_array['indent'].$blt_array['text'][$i],$border,$align,$fill);
