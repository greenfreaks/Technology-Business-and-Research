...
if ($numNivelesPocoAvance>0){
	...
	GROUP BY trl_nominacion_producto.nombre;";
...
//texto descriptivo
if($nivelAlcanzado==0){
	//Texto 11  OPC1 conclusiones 
	darFormatoTexto('En las páginas previas se explicó la importancia de la escala TRL y los niveles que la conforma, el avance mínimo (si lo tuvo) en cada nivel. Pero como en toda evaluación no es suficiente con una ponderación, la información más importante es la retroalimentación, la cual para este estudio dividimos en cursos o servicios acorde a las necesidades de la tecnología y acciones que permitirán al equipo madurar la tecnología propuesta.
	
	El proceso para convertir una tecnología en un producto comerciable requiere no sólo de conocimientos técnicos si no de aspectos empresariales, de producción y comerciales, que no siempre son dominados por los miembros del equipo de trabajo, de ahí la importancia del acompañamiento en el proceso de desarrollo tecnológico.
	
	TB&R es una empresa comprometida con la transformación del conocimiento científico y tecnológico como medio para brindar bienestar social, económico y ambiental al país. Por tal motivo se exhorta a académicos, empresarios o emprendedores a darle seguimiento a sus ideas y proyectos en favor del sistema innovador nacional. Se invita a visitar la página oficial de TB&R (http://www.techbusiness.com.mx/) para conocer los beneficios de trabajar con su equipo especializado. Para dar seguimiento al proyecto conserve este reporte o considere abordar los temas de cursos y servicios mostrados en la sección promedio de aspectos cumplidos.');	
}else if ($nivelAlcanzado<9){

	//Texto 11  OPC2 conclusiones 	
	darFormatoTexto('En las páginas previas se explicó la importancia de la escala TRL, los niveles que la conforma y un desglose detallado del porque el proyecto "'.$tituloProyecto.'" se ubica en un TRL '. $nivelAlcanzado.'. Pero como en toda evaluación no es suficiente con una ponderación, la información más importante es la retroalimentación, la cual para este estudio dividimos en cursos o servicios acorde a las necesidades de la tecnología, evidencias que sustenta esta evaluación y acciones a tomar para avanzar hacia un TRL más alto.
	
	El proceso para convertir una tecnología en un producto comerciable requiere no sólo de conocimientos técnicos si no de aspectos empresariales, de producción y comerciales, que no siempre son dominados por los miembros del equipo de trabajo, de ahí la importancia del acompañamiento en el proceso de desarrollo tecnológico.
	
	TB&R es una empresa comprometida con la transformación del conocimiento científico y tecnológico como medio para brindar bienestar social, económico y ambiental al país. Por tal motivo como la última aportación de este informe se mencionan algunos cursos y servicios en los que somos especialistas y que se podrían requerir para que el proyecto "'.$tituloProyecto.'" ascienda en la escala TRL:
	');

	//encabezados tabla cumplimiento por categoría
	darFormatoTextoEncabezadoTabla();
	//$pdf->Ln(3);
	$pdf->SetX(20);
	$pdf->Cell(100,ANCHORENGLON,'PRODUCTO',0,0,'C',1);
	$pdf->Cell(70,ANCHORENGLON,utf8_decode('MAS INFORMACIÓN...'),0,1,'C',1);

	//contenido tabla  complimiento por categoría
	darFormatoTextoContenidoTabla();
	$yPosterior = $pdf->getY();
	while($row = $resultado10->fetch_assoc())
	{
		// se usa la variable $yPosterior debido a que la prima multicell siempre será más alta que la primera
		//de no agregarse se sobre escribirian los texto cuando estos usen más de un renglon
		$pdf-> SetXY(20,$yPosterior);
		$yAnterior=$pdf->getY(); 
		if ($row['trl_tipos_producto_idtrl_tipos_producto']==1){
			$pdf->multicell(100,ANCHORENGLON,'Curso de "'.utf8_decode($row['nombre']).'"',"T","L",0);

		}else{
			$pdf->multicell(100,ANCHORENGLON,'Servicio de "'.utf8_decode($row['nombre']).'"',"T","L",0);	
		}
		$yPosterior = $pdf->getY();
		//se suma el ancho de la multicell anterior (+10 de margen) al eje X
		//se usa el valor de Y de antes de dibujar la multicell anterior
		// para dibujar la siguente multicell en el mismo renglon
		$pdf->setXY($pdf->getX()+110,$yAnterior);
		$pdf->multicell(70,ANCHORENGLON,$row['enlace'],"T","C",0);
	}

	//titulo tabla descripción niveles
	darFormatoTitulosFiguras ('Talleres y cursos para mejorar TRL','t');
	$pdf->ln(2);

}else{
	//Texto 11  OPC3 conclusiones 
	darFormatoTexto('En las páginas previas se explicó la importancia de la escala TRL, los niveles que la conforma y un desglose detallado del porque el proyecto "'.$tituloProyecto.'" se ubica en un TRL '. $nivelAlcanzado.'. Pero como en toda evaluación no es suficiente con una ponderación. la información más importante es la retroalimentación, la cual para este estudio dividimos en cursos o servicios acorde a las necesidades de la tecnología y evidencias que sustenta esta evaluación.
	
	TB&R es una empresa comprometida con la transformación del conocimiento científico y tecnológico como medio para brindar bienestar social, económico y ambiental al país. Por tal motivo se presenta como un aliado para llevar el producto aquí evaluado a los mercados más relevantes, un intermediario para gestionar colaboraciones con otras empresas, universidades o centros de investigación y como acelerador en la generación de nuevos productos o servicios. Se invita a consultar la página oficial de la empresa para consolidar más ideas en realidad:  http://www.techbusiness.com.mx/
');
}
//---------sección 6-----------
$pdf->addPage();
